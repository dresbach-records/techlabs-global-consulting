        ...
        <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
          <div className="lg:w-3/4">
            <div className="inline-flex items-center gap-4 mb-8">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#19C37D]">Global Architectural Authority</span>
            </div>
            <h1 className="text-6xl md:text-[110px] font-black tracking-tighter leading-[0.8] mb-12 uppercase">
              Precision <br />
              <span className="text-[#19C37D]">Systems</span> <br />
              Engineering
            </h1>
      ...
      <section className="py-40 bg-black">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row justify-between items-end mb-24 gap-12">
            <div>
              <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#19C37D] mb-8">Consulting Capabilities</h2>
              <h3 className="text-5xl md:text-7xl font-black tracking-tighter uppercase leading-[0.9]">
                Specialized <br /> <span className="text-[#6B6B6B]">Business</span> <br /> Architecture
              </h3>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-[#2F2F2F] border border-[#2F2F2F]">
            <div className="bg-black p-12 lg:p-24">
              <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#19C37D] mb-16">Corporate Center</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
                {CORPORATE_LINKS.map(link => (
                  <Link key={link.title} to={`/${link.path}`} className="group cursor-pointer">
                    ...
                    <p className="text-[10px] text-[#2F2F2F] font-mono group-hover:text-[#6B6B6B] transition-colors uppercase tracking-widest">Global Ops Node</p>
                  </Link>
                ))}
              </div>
            </div>
            <div className="bg-black p-12 lg:p-24 lg:border-l border-[#2F2F2F]">
              <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#19C37D] mb-16">Governance & Ethics</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
                {GOVERNANCE_LINKS.map(link => (
                  <Link key={link.title} to={`/${link.path}`} className="group cursor-pointer">
                    <div className="text-[#6B6B6B] group-hover:text-[#19C37D] transition-colors mb-6">{link.icon}</div>
                    <h4 className="font-black uppercase tracking-tighter text-sm mb-2">{link.title}</h4>
                    <p className="text-[10px] text-[#2F2F2F] font-mono group-hover:text-[#6B6B6B] transition-colors uppercase tracking-widest">Policy Compliance</p>
                  </Link>
                ))}
              </div>
            </div>
