...
export default function TechAudit() {
  const data = DETAILED_PAGES['tech-audit'];
  return (
        ...
        <div className="max-w-7xl mx-auto px-6">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#2F2F2F] border border-[#2F2F2F]">
              {data.content.map(item => (
                 <div key={item} className="p-12 bg-black hover:bg-[#0A0A0A] transition-all group">
                    <h3 className="text-xl font-black uppercase group-hover:text-[#19C37D] transition-colors">{item}</h3>
                 </div>
              ))}
           </div>
        </div>
